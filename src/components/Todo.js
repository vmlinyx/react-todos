...
const Todo = function ({ text, todo, todos, setTodos }) {
  ...
  return (
    <div className="todo">
      ...
      <button onClick={completeHandler} className="complete-btn">{todo.completed ? "redo" : "check"}</button>
      <button onClick={deleteTodoHandler} className="trash-btn">trash</button>
    </div>
  );
}
